...
def scaleXTune():
    for BC in numpy.arange(0.1, 10, 0.5):
        for DC in numpy.arange(0.1, 10, 0.5):
            ...
            c1.setName("ScaleX")
            nodetest=NodeTest(horizon, [c1], monitoringWindow,
                            Application1(sla=appSLA, stime=stime, init_cores=initCores))
            runner = RunnerTest(nodetest)
...
for st in setpoints:
    ...
    nodetest = NodeTest(horizon, [c0, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10], monitoringWindow, Application1(appSLA), genMonitoring = None, name="run1")
    runner = RunnerTest(nodetest) # Insert a list of Nodes
