    ...
    def plot(self, isTotalRT = True):
        ...
        for monitoring, name in zip(self.monitorings, self.names):
            ...
            for (rts, cores, users) in zip(arts, acores, ausers):
                ...
                fig.tight_layout()
                plt.savefig("experiments/neptune/%s-%d-workcore.pdf" % (name, i))
                plt.close()
                ...
                ax2.set_ylim([m, M])
                fig.tight_layout()
                plt.savefig("experiments/neptune/%s-%d-%s" % (name, i, rt_label))
                plt.close()
                i += 1
