...
class MainComplaxApp:
    ...
    def run(self, filename=''):
        ...
        i = 0
        for sla in slas:
            apps.append(ApplicationModel(appSLA, init_cores=initCores, nominalRT=nominalRT[i]))
            st = alfa * nominalRT[i] / total_weight
            localSLA = slas[i] if slas[i] > 0.0 else nominalRT[i] * 2
            if localSLA*5/9 < st*appSLA or localSLA/2 > st*appSLA:
                if localSLA * 5 / 9 < st * appSLA:
                    print(st * appSLA, '>', localSLA * 5 / 9)
                else:
                    print(st * appSLA, '<', localSLA / 2)
                st = 0.5 * localSLA / appSLA

            c = CTControllerScaleDependency(period, initCores, BC=0.001, DC=0.02, st=st); c.setName("ScaleX")
            controls.append(c)
            monitors.append(Monitoring(monitoringWindow, appSLA, local_sla=sla))
            controls[i].setSLA(appSLA)
        ...
        controls[0].setGenerator(generators[0]); controls[0].setName("RP -f1")
        ...
        for app in apps:
            dg_dep.nodes["f"+str(i+1)]['node'] = Node(horizon, controls[i], app, monitoring=monitors[i], name="f"+str(i+1),
                                                    generator=generators[i], local_sla=slas[i])
            i = i+1

        for node in dg_dep:
            dg_dep.nodes[node]['users'] = 0
            dg_dep.nodes[node]['node'].total_weight = 2.386  # given by user
            dg_dep.nodes[node]['node'].subtotal_weight = dg.nodes[node]['node'].app.weight

        simul = SimulationWithDependeciesDAG(horizon, dag_model, 'f1')
        simul.run()
        simul.plot()
        ...
        dag_model.print_dag('users',  pos=positions,show_sync=True, show_times=True,label='dependency-normal') # or label='dependency-bottleneck'
...
for i in range(11):
    MainComplaxApp().run("experiments/neptuneplus/%s-%d" % ("complexapp/statistical", i))

ComplexDAG(nx.DiGraph(), 'Ord').computeResultTable(simulatorLebal="neptuneplus/complexapp/statistical", timeWindow=1200)
